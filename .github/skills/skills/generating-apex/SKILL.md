...
- Framework reference to follow `references/generating-apex-framework-reference.md` for a new service class
...
3. **Review templates, framework, and PMD guidelines**
...
6. **Run PMD code analyzer**
   ...
   - **Remediate ALL violations** before reporting:
     ...
     - **Sev 3 (Minor)**: Naming conventions, documentation — SHOULD FIX but may report as warning
   - Re-run until output shows: **Total Issues: 0** or only acknowledged warnings
   - Capture tool output verbatim for the report
   - Fallback (if `sf scanner` unavailable): Use `sf code-analyzer run --target <target>` 
...
### Phase 2.5 — PMD Compliance Checklist (Self-Review Before Step 6)

Before running the scanner, **manually verify** these common violations:

| PMD Rule | What to Check | Auto-Fix |
|---|---|---|
| **AvoidDmlStatementsInLoops** | No `insert/update/delete/upsert` inside loops | Batch outside loop or use `Database.executeBatch()` |
| **AvoidSoqlStatementsInLoops** | No `SELECT ... FROM` inside loops | Query once before loop; use `Map<Id, SObject>` for lookups |
| **CyclomaticComplexity** | Method has ≤10 decision points (if/else/switch/for) | Extract conditional logic into private helper methods |
| **ExcessiveClassLength** | Class has ≤1000 lines | Split into smaller Service/Selector/Domain classes |
| **ExcessiveMethodLength** | Method has ≤400 lines | Extract logical sections into `private` helpers |
| **ExcessiveParameterList** | Method has ≤3 parameters | Group into wrapper object (DTO/wrapper class) |
| **ClassNamingConventions** | Class starts with uppercase letter (PascalCase) | Rename: `MyService` not `myservice` |
| **MethodNamingConventions** | Method starts with lowercase letter (camelCase) | Rename: `getAccounts()` not `GetAccounts()` |
...
**Best Practice**: After authoring, ctrl+click on each method and visually scan for these patterns before running the automated scan.
...
* extract each condition into `isXEligible()`
* extract business branches into `processX()`
* use strategy dispatch over nested if/else
* max nesting depth = 3
* split method beyond 40 NCSS
* split class beyond 500 NCSS
* target cognitive complexity < 15
* target cyclomatic complexity < 10

---

## B) Public surface + size control
...
* public methods only for orchestration API
* private helpers for transformations
* max public surface target < 20
* max field target < 15
* group related fields into wrappers
* split large utility classes into focused modules
