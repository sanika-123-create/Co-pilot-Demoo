...
# PMD-Driven Test Generation Rules

This section translates PMD test rules into active behavior.

## A) Required assertions
...
* every test method must contain meaningful assertions
* every assert must include a descriptive failure message
* prefer outcome assertions over non-null assertions
* validate field values, side effects, DML counts, and emitted async artifacts

### Assertion pattern

```apex
System.assertEquals(expectedStatus, actualStatus,
    'Shipment status should be updated after successful processing.');
```
...
* include at least one `System.runAs(testUser)` per major test class
* use persona-specific users from factory helpers
* validate CRUD/FLS-sensitive logic in user context
* use admin + restricted user comparison where business-critical

---

## C) Annotation & isolation standards
...
* keep helper methods private static
* no production utility methods inside test class
* isolate all org dependencies with factories or mocks

---

## D) Complexity control for tests

Even tests must avoid:
...
* split bulky tests into focused methods
* extract repeated verification into private assertion helpers
* use scenario builders instead of deeply nested setup
* max nesting depth = 2 in tests
